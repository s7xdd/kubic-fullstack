...
@section('content')
    <section class="relative min-h-screen bg-black  text-white flex items-center justify-center overflow-hidden px-4">
        ...
        <div x-data="{ tab: 'login', show: false }"
            class="relative z-20 bg-zinc-900/50   backdrop-blur-sm shadow-2xl p-8 w-full max-w-xl transition-all">
            <div class="flex border-b border-gray-800 mb-8">
                <button
                    :class="tab === 'login' ? 'border-white text-white' :
                ...
                <button
                    :class="tab === 'request' ? 'border-white text-white' :
                        'border-transparent text-gray-500 hover:text-white'"
                    @click="tab = 'request'"
                    class="flex-1 text-center py-2 border-b-2 font-medium uppercase tracking-wide transition-all">
                    Request Access
                ...
                <form method="GET" action="{{ route('projects') }}">
                    <div class="mb-6 relative">
                        ...
                        <button type="button" @click="show = !show"
                            ...
                            <svg x-show="show" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.477 0-8.268-2.943-9.542-7a9.978
